...
def set_pose(robot, pose):
  ...
  for j in range(num_joints):
    ...
    if (j_pose_size > 0):
      j_pose = pose[j_pose_idx:(j_pose_idx + j_pose_size)] # order: FR hip, upper, lower, FL..., RR, RL, 
...
def main(argv):
  ...
  p.configureDebugVisualizer(p.COV_ENABLE_SINGLE_STEP_RENDERING, p.COV_ENABLE_MOUSE_PICKING, p.COV_ENABLE_GUI, 1)
